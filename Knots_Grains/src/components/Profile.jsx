...
import { setPostImageUrls } from "../store/Authslice";
...
export default function CarpenterProfile() {
  ...
  useEffect(() => {
    if (!authStatus) navigate("/login");
      ...
      setSkills(useSelector((state) => state.auth.skills)); // load initial skills

      const fetchImages = async () => {
        try {
          const imageUrls = await authService.getPostImageUrls(carpenter.email);
  ...
  const handleAddSkill = () => {
    if (newSkill.trim() && !skills.includes(newSkill.trim())) {
      ...
      setNewSkill("");
      // TODO: Call backend to update skills in DB
    }
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    formData.append("email", carpenter.email);
      ...
      <Card className="max-w-4xl w-full shadow-xl rounded-2xl overflow-hidden">
        <CardContent className="p-6 md:p-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 text-gray-700">
            <p>
              <strong>Experience:</strong> {carpenter.experience}
            </p>
          ...
          <div className="mt-6">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-xl font-semibold text-amber-800">Specialties</h2>
              <Button
            ...
            {editSkillsMode && (
              <div className="flex gap-2 mt-2">
                ...
                />
                <Button onClick={handleAddSkill} className="bg-amber-500 hover:bg-amber-600 text-white">
                  Add
                </Button>
              </div>
            )}
